...
public class ApiDialog extends BaseDialog {
    ...
    public ApiDialog(@NonNull @NotNull Context context) {
        ...
        // 直播源：优先读 Hawk 存储，若为空则使用默认值
        String savedLiveUrl = Hawk.get(HawkConfig.LIVE_URL, "");
        if (savedLiveUrl.isEmpty()) {
            savedLiveUrl = "https://ghfast.top/raw.githubusercontent.com/Supprise0901/TVBox_live/main/live.txt";
            Hawk.put(HawkConfig.LIVE_URL, savedLiveUrl);
        }
        liveApi.setText(savedLiveUrl);
