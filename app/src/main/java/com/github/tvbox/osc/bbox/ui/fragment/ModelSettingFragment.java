...
//        tvApi.setText(Hawk.get(HawkConfig.API_URL, ""));
        ...
        findViewById(R.id.llHomeApi).setOnClickListener( v -> {
            ...
            // 若没有历史记录，直接打开 API 输入框，让用户添加新源
            if (history.isEmpty()) {
                ApiDialog addDialog = new ApiDialog(mActivity);
                EventBus.getDefault().register(addDialog);
                addDialog.setOnListener(url -> {
                    if (-1 != url.indexOf("api-")) {
                        url = url.replaceAll("api-", "");
                        tvHomeApi.setText(url);
                    }
                });
                addDialog.setOnDismissListener(d -> EventBus.getDefault().unregister(d));
                addDialog.show();
                return;
            }
...
//                        tvApi.setText(api);
    ...
    private void checkUpdate () {
        ...
        XUpdate.newBuild(mContext)
                ...
                .updateParser(new CustomUpdateParser(mContext))
