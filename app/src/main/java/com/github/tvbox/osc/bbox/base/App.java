...
public class App extends MultiDexApplication {
    ...
    private void putDefaultApis() {
        ...
        // 添加默认直播源
        // 若历史安装遗留 LIVE_URL=""，强制覆盖为默认值
        String savedLiveUrl = Hawk.get(HawkConfig.LIVE_URL, "");
        if (savedLiveUrl == null || savedLiveUrl.isEmpty()) {
            Hawk.put(HawkConfig.LIVE_URL, URL.DEFAULT_LIVE_URL);
        }

        putDefault(HawkConfig.DEFAULT_STORE_API, defaultStoreApi);
        // 添加默认多源（多仓）地址到 STORE_API_MAP，让用户打开"多源配置"即看到
        if (!Hawk.contains(HawkConfig.STORE_API_MAP)) {
            HashMap<String, String> defaultStoreMap = new HashMap<>();
            String defaultMultiUrl = URL.DEFAULT_STORE_API_URL;
            defaultStoreMap.put(defaultMultiUrl, defaultMultiUrl);
            Hawk.put(HawkConfig.STORE_API_MAP, defaultStoreMap);
            Hawk.put(HawkConfig.STORE_API_NAME, defaultMultiUrl);
            ArrayList<String> defaultStoreNameHistory = new ArrayList<>();
            defaultStoreNameHistory.add(defaultMultiUrl);
            Hawk.put(HawkConfig.STORE_API_NAME_HISTORY, defaultStoreNameHistory);
        }
